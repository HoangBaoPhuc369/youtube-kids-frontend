...
export default function ChannelVideoAdmin() {
  const dispatch = useDispatch();

  const { childrenActive } = useSelector((state) => state.auth);

  const { videos, channelVideo, loading, error } = useSelector(
    (state) => state.video
  );
  ...
  return (
    <div className="home-wrapper background-channel">
      ...
      <div className="home-container">
        ...
        <Video videos={videos} loading={loading} error={error} role="admin" />
      </div>
